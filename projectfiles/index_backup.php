   ...
   $(document).on('change','#editalert',function () {
    if($('#editalert').val()=="Anychange")
      ...
      $(document).on('click', '.btn_properties', function(){
        ...
        var tr = $(this).parents("tr");
        var id=table.row($(this).closest('tr')).data()['ID'];
        var url=tr.find("td").eq(1).text();
        var depth=tr.find("td").eq(2).text();
        var Alert=tr.find("td").eq(4).text();
        var Cronvalue=tr.find("td").eq(5).text();
        var Results = tr.find("td").eq(6).text();
      ...
      $(document).on('click', '.startcrawl', function(){
       ...
       $.ajax({
          ...
          dataType:"text",
          //async:false,
          beforeSend: function () {tr.find("td").eq(3).text("pending");},
          success:function(response)
          {
              var index_value=this.indexValue;
              if(response=="no new content added")
              {tr.find("td").eq(6).text("No change");}
              else if(response=="no new content added for searched keywords")
              {
                tr.find("td").eq(6).text("No new results found for specified keywords");
              }
              else if(response.indexOf("The following page has changed!") !== -1){
                tr.find("td").eq(6).text("New results");
              }
              else if(response.indexOf("childurl not Inserted") !== -1){
                tr.find("td").eq(6).text("Error Occured");
              }
              else if(response=="page not found"){
                tr.find("td").eq(6).text("Error occured (404)");
              }
              else{
                tr.find("td").eq(6).text("Error occured (Time out)");
              }
              var data1=this.link;var data2=this.depth;var data3=response;
              setTimeout(function () {tr.find("td").eq(3).text("complete");}, 1000);
              $.ajax({
